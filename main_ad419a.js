(function () {
    ...
    // export default () => {
    //   const blocks = document.querySelectorAll('.faq, .efficacy');
    //
    //   blocks.forEach(block => {
    //     const itemSelector = block.classList.contains('faq') ? '.faq__item' : '.efficacy__item';
    //     const headerSelector = block.classList.contains('faq') ? '.faq__header' : '.efficacy__header';
    //     const toggleSelector = block.classList.contains('faq') ? '.faq__toggle' : '.efficacy__toggle';
    //     const contentSelector = block.classList.contains('faq') ? '.faq__content' : '.efficacy__content';
    //
    ...
    //       } else {
    //         // Аккордеон: закрываем другие
    //         block.querySelectorAll(itemSelector).forEach(otherItem => {
    //           if (otherItem !== item) {
    ...
    //     }
    //
    //     // Навешиваем обработчик на заголовки
    //     block.querySelectorAll(headerSelector).forEach(header => {
    ...
    //     });
    //
    //     // Навешиваем обработчик на кнопки toggle
    //     block.querySelectorAll(toggleSelector).forEach(button => {
    //       button.addEventListener('click', e => {
    //         e.stopPropagation(); // чтобы клик на кнопке не "дошёл" до заголовка и не сработал дважды
    ...
    //     const animateSteps = (containerSelector, itemSelector, animatedClass = 'animated', totalDelay = 400) => {
    ...
    //             });
    //
    //             // Ждём пока последний элемент анимируется и добавляем класс для ::before
    //             setTimeout(() => {
    //               container.classList.add('line-full');
    //             }, children.length * totalDelay + 100); // +100 — небольшой запас
    //
    //             observer.unobserve(container); // Только один раз
    //           }
    //         });
    //       }, {
    //         threshold: 0.1
    ...
    var scrollSmooth = (function () {
      ...
      function handleLink(event) {
        ...
        var elem = document.querySelector(target.getAttribute('href'));
        console.log(elem);
        window.scrollTo({
    ...
    var promo = (function () {
      ...
      //
      //   // добавляем .active при первом наведении
      ...
      //     // el.addEventListener('touchmove', zoom);
    ...
    //
    //     // Вычисляем индекс активной группы
    //     const activeGroupIndex = Math.floor(swiper.realIndex / slidesPerGroup);
    //
    //     const isSingleSlide = getCurrentSlidesPerView(swiper) === 1;
    //
    //     if (!isSingleSlide) {
    //       bullets.forEach((el, i) => {
    //         el.style.display = 'inline-block';
    //         el.classList.toggle('swiper-pagination-bullet-active', i === activeGroupIndex);
    //       });
    //       return;
    //     }
    //
    //     // Mobile — maxDots
    //     const start = Math.max(0, Math.min(totalGroups - maxDots, activeGroupIndex - Math.floor(maxDots / 2)));
    //     const end = Math.min(totalGroups, start + maxDots);
    //
    //     bullets.forEach((el, i) => {
    ...
    //     // В зависимости от breakpoints возвращает slidesPerView как число
    //     const width = window.innerWidth;
    //     const breakpoints = swiper.params.breakpoints;
    //     let matched = 0;
    //     for (let bp in breakpoints) {
    //       if (width >= bp) matched = bp;
    //     }
    //     const bpSettings = breakpoints[matched] || {};
    //     return bpSettings.slidesPerView || swiper.params.slidesPerView || 1;
    //   }
    // };
